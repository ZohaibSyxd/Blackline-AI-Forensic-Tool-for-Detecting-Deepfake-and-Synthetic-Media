...
def aggregate_shot(scores: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    # mean per (asset_id, shot_index)
    agg: Dict[Tuple[Any, Any], List[float]] = {}
    for r in scores:
        key = (r.get("asset_id"), r.get("shot_index"))
        agg.setdefault(key, []).append(r["score_fake"])
    out = []
    for (asset_id, shot_idx), vals in agg.items():
        out.append({
            ...
            "shot_index": shot_idx,
            "model": "xception_timm",
            "score_fake": float(sum(vals)/len(vals)),
            "frames": len(vals),
        })
    return out
...
def main():
    ...
    if args.aggregate_shot_out:
        shots = aggregate_shot(per_frame)
