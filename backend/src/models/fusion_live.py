...
def _sample_frames_video(vpath: Path, num: int) -> List[Image.Image]:
    if HAS_DECORD:
        try:
            ...
            return [Image.fromarray(fr) for fr in arr]
        except Exception:
            pass
    # Fallback: OpenCV
...
def _predict_xception_agg(vpath: Path, checkpoint: Optional[Path], frames: int = 16, agg: str = "p95") -> float:
    det = _ensure_xception(checkpoint)
    imgs = _sample_frames_video(vpath, frames)
    if not imgs:
        return 0.5
...
def predict_fusion_single(video_path: Path, models_root: Path | None = None) -> Dict[str, object]:
    ...
    try:
        xs = _predict_xception_agg(video_path, x_ckpt if x_ckpt.exists() else None, frames=24, agg="p95")
    except Exception:
        xs = 0.5
    try:
